...
param( 
    ...
    try{
        ...
        $ServerRules = Get-AzureRmSqlServerFirewallRule -ResourceGroupName $ResourceGroup -ServerName $SqlServer | Select -Property FirewallRuleName
    
        $RulesToDelete = $ServerRules | where {$_.FirewallRuleName -notin $RulesNotToDelete}
    
        Write-Output "Removing selected rule"
        foreach ($RuleName in $RulesToDelete)
        {
            $PurgedRules = "$PurgedRules $RuleName"
            Remove-AzureRmSqlServerFirewallRule -FirewallRuleName $RuleName.FirewallRuleName -ResourceGroupName $ResourceGroup -ServerName $SqlServer
        }
